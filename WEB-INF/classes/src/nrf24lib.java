...
public class nrf24lib{
	...
	public void flush_rx(){
		try{spi.write(FLUSH_RX);}catch(IOException e){ e.printStackTrace(); }
	}

	public void flush_tx(){
		try{spi.write(FLUSH_TX);}catch(IOException e){ e.printStackTrace(); }
	}
	...
	public byte getDynamicPayloadSize(){
		...
		byte[] result=null;
		try{ result = spi.write(data); }catch(IOException e){e.printStackTrace();}
		return result[1];
	}
	...
	public void enableDynamicPayloads(){
		...
		write_register(FEATURE,read_register(FEATURE) | BV(EN_DPL) );
		...
		write_register(DYNPD,read_register(DYNPD) | BV(DPL_P5) | BV(DPL_P4) | BV(DPL_P3) | BV(DPL_P2) | BV(DPL_P1) | BV(DPL_P0));
	...
	public void enableAckPayload(){
		...
		//
		write_register(FEATURE,read_register(FEATURE) | BV(EN_ACK_PAY) | BV(EN_DPL) );
		...
		write_register(DYNPD,read_register(DYNPD) | BV(DPL_P1) | BV(DPL_P0));
	}
	...
	public void setRetries(int delay, int count){
		write_register(SETUP_RETR,((char)delay&0xf)<<ARD | ((char)count&0xf)<<ARC);
	}
}